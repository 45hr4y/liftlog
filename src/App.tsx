...
type Routine = { id?: number; name: string; color: string; archived?: boolean; favourite?: boolean; favorite?: boolean; pinned?: boolean; createdAt: string };
  ...
  return <section className="homeV12">
    ...
    <div className="homeWeeklyCards">
      <Card cls="continueRoutineCard">
        ...
        <p className="muted">{scheduledRoutine ? 'Scheduled for today.' : (favouriteRoutine ? 'Your chosen favourite routine.' : 'No favourite set yet — using suggested routine.')}</p>
...
function RoutinesPage({data}:any){
  ...
  async function setFavouriteRoutine(id:number){
    await db.transaction('rw', db.routines, async()=>{
      ...
      for(const r of all) if(r.id) await db.routines.update(r.id,{favourite:r.id===id, favorite:r.id===id, pinned:r.id===id});
    });
    refresh();
  }
